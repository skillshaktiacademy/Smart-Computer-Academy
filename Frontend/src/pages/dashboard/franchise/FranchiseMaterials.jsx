...
import { 
  Box, Plus, Search, FileText, 
  Video, Download, Trash2, Edit3,
  Loader2, X, CheckCircle2, BookOpen,
  Upload, Filter, Globe, Lock
} from "lucide-react";
...
const FranchiseMaterials = () => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [file, setFile] = useState(null);
  const queryClient = useQueryClient();

  const { data: materialsData, isLoading } = useQuery({
    queryKey: ["franchise-materials"],
    queryFn: () => api.get("/materials/my-center"),
  });
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries(["franchise-materials"]);
  ...
  return (
    <div className="space-y-8 pb-12">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-gray-900">Study Materials</h1>
          <p className="text-gray-500 font-medium">Upload and manage digital resources for your students.</p>
        </div>
        <button 
              ...
              <div className="p-8">
                ...
                <form onSubmit={handleSubmit(mutation.mutate)} className="space-y-6">
                  <div>
                    ...
                    <input {...register("title")} className="input-field py-3" placeholder="e.g. Introduction to MS Excel" />
                  ...
                  <div className="space-y-4">
                    <label className="block text-xs font-black text-gray-400 uppercase tracking-widest">Select File</label>
                    <div className="border-4 border-dashed border-gray-100 rounded-3xl p-10 text-center hover:border-primary/20 transition-all cursor-pointer relative overflow-hidden group">
                      ...
                      <div className="flex flex-col items-center">
                        ...
                        <p className="text-sm font-bold text-gray-600">{file ? file.name : "Drop file or click to browse"}</p>
                        <p className="text-[10px] text-gray-400 mt-2 font-bold uppercase tracking-widest">Max size: 20MB</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 flex gap-4">
                    ...
                    <button type="submit" disabled={mutation.isLoading} className="flex-1 btn-primary py-4 flex items-center justify-center gap-2">
                      {mutation.isLoading ? <Loader2 className="animate-spin" size={20} /> : <><CheckCircle2 size={20} /> <span>Upload Resource</span></>}
                    </button>
                  </div>
                </form>
              </div>
...
export default FranchiseMaterials;
